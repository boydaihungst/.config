...
if haveDepCtrl then
	...
	depRec = DependencyControl({
		feed = "https://raw.githubusercontent.com/unanimated/luaegisub/master/DependencyControl.json",
	})
end
...
function fucks(subs, sel)
	for z, i in ipairs(sel) do
		...
		if not t:match("\\i?clip%(m") and not t:match("//i?clip%(m") then
			t_error("Missing \\clip on line #" .. i - line0 .. ".", 1)
		end
		cx1, cy1, cx2, cy2, cx3, cy3, cx4, cy4 =
			t:match("clip%(m ([%d%-]+) ([%d%-]+) l ([%d%-]+) ([%d%-]+) ([%d%-]+) ([%d%-]+) ([%d%-]+) ([%d%-]+)")
		if not cx1 then
			cx1, cy1, cx2, cy2 = t:match("clip%(m ([%d%-]+) ([%d%-]+) l ([%d%-]+) ([%d%-]+)")
		end
		if not cx1 then
			t_error("Line #" .. i - line0 .. ": Not enough clip points. 2 required.", 1)
		end
			...
			if res.grad then
				...
				for c = 2, #chars do
					if chars[c] == " " then
						...
						tt = tt .. "{\\fax" .. round((faks + faxdiff * (c - 1)) * 100) / 100 .. "}" .. chars[c]
					end
				end
				t = tg .. tt
				if orig:match("{%*?\\") then
...
function frozt(subs, sel)
	for z, i in ipairs(sel) do
		...
		local l = subs[i]
		local text = l.text
		if not text:match("\\i?clip%(m") and not text:match("//i?clip%(m") then
			t_error("Missing \\clip on line #" .. i - line0 .. ".\nAborting.", 1)
		end
		cx1, cy1, cx2, cy2, cx3, cy3, cx4, cy4 =
			text:match("clip%(m ([%d%-]+) ([%d%-]+) l ([%d%-]+) ([%d%-]+) ([%d%-]+) ([%d%-]+) ([%d%-]+) ([%d%-]+)")
		if not cx1 then
		...
		text = text:gsub("\\i?clip%b()", ""):gsub(ATAG, function(tg)
			return duplikill(tg)
		end)
		l.text = text
		subs[i] = l
...
function necrostuff(subs, sel)
	...
	copyshit = {
		...
		{ x = 3, y = 0, class = "label", label = "    Place * in text below to copy tags there" },
		{
			...
			class = "checkbox",
			name = "breaks",
			label = "Copy tags after all linebreaks (all lines)",
			realname = "",
		},
		{
			x = 0,
			...
			name = "ltxt",
			value = vis,
			hint = "only works for first selected line",
		},
	}
	ftw = 3
	-- regular tags -> GUI
	for f in ftags:gmatch("\\[^\\}]+") do
		lab = f:gsub("&", "&&")
		if f:match("\\i?clip%(m") then
			lab = f:match("\\i?clip%(m [%d%.%-]+ [%d%.%-]+ %a [%d%.%-]+ [%d%.%-]+") .. " ..."
		end
		if f:match("\\move") then
			lab = f:gsub("%.%d+", "")
		end
	...
	if cstext:match("{[^}]-\\[^Ntrk]") then
		for f in cstext:gmatch("\\[^tNhrk][^\\})]+") do
			lab = f:gsub("&", "&&")
			if itw == 22 then
				lab = "(that's enough...)"
				f = ""
	...
	repeat
		if press == "Check All Tags" then
			for key, val in ipairs(copyshit) do
				if val.class == "checkbox" and not val.label:match("%[ ") and val.x == 0 then
					val.value = true
				end
			end
		end
		...
		press, rez = ADD(
			copyshit,
			{ "Copy", "Check All Tags", "Paste Saved", "[Un]hide", "Cancel" },
			{ ok = "Copy", close = "Cancel" }
		)
	...
	for key, val in ipairs(copyshit) do
		if rez[val.name] == true and not val.label:match("%[") then
			if not val.label:match("\\t") then
				kopytags = kopytags .. val.realname
			else
				copytfs = copytfs .. val.label
			end
		end
	end
	if press == "Paste Saved" then
		kopytags = savedkopytags
		copytfs = savedcopytfs
		sn = 1
		csstyle = savedstyle
		csst = savedt1
		cset = savedt2
	...
	if rez.ltxt:match("%*") then
		inline = true
		sn = 1
		maxx = 1
	elseif rez.breaks then
		inline = true
		sn = 1
		maxx = #sel
	else
	...
	if press ~= "[Un]hide" then
		for z = sn, maxx do
			...
			if inline then
				...
				if visible ~= visible2 then
					logg(
						"Line #"
							.. i - line0
							.. ": It appears that characters have been lost or added. This is probably a failure of the re module."
					)
				end
			else
			...
			-- add transforms
			if trnsfrm then
				text = text:gsub("^({\\[^}]*)}", "%1" .. trnsfrm .. "}")
			end
			trnsfrm = nil
			text = text:gsub(STAG, function(tags)
				return cleantr(tags)
			end)
			...
			if rez.chks then
				line.start_time = csst
			end
			if rez.chke then
				line.end_time = cset
			end
			if rez.tkst then
				text = text:gsub("^({\\[^}]-}).*", "%1" .. cstext)
			end
			line.text = text
			subs[i] = line
		end
		...
		if kopytags == "" and copytfs == "" then
			...
			for com in txt:gmatch("{//([^}]+)}") do
				table.insert(uncom, { x = 0, y = wai, class = "checkbox", label = com, name = com })
				wai = wai + 1
			end
			if #uncom <= 1 then
		...
		-- hide
			...
			for tg in ekopytags:gmatch("\\[^\\}]+") do
				txt = txt:gsub(tg, "")
			end
			for tg in kopytags:gmatch("\\([^\\}]+)") do
				txt = txt .. "{//" .. tg .. "}"
			end
			for tg in ecopytfs:gmatch("\\t%%%b()") do
				txt = txt:gsub(tg, "")
			end
			for tg in copytfs:gmatch("\\(t%b())") do
				txt = txt .. "{//" .. tg .. "}"
			end
			txt = txt:gsub("{}", "")
		end
...
function copycolours(subs, sel)
	...
	if not res.c1 and not res.c2 and not res.c3 and not res.c4 then
		t_error("No colours selected", 1)
	end
	local col = {}
	for n = 1, 4 do
		if res["c" .. n] then
	...
	for z, i in ipairs(sel) do
		...
		if z == 1 then
			...
			if res.c1 then
				C.col1 = nontra:match("\\c(&H%x+&)") or sr.color1:gsub("H%x%x", "H")
				C.alf1 = nontra:match("\\1a(&H%x+&)") or sr.color1:match("&H%x%x") .. "&"
			end
			if res.c3 then
				C.col3 = nontra:match("\\3c(&H%x+&)") or sr.color3:gsub("H%x%x", "H")
				C.alf3 = nontra:match("\\3a(&H%x+&)") or sr.color3:match("&H%x%x") .. "&"
			end
			if res.c4 then
				C.col4 = nontra:match("\\4c(&H%x+&)") or sr.color4:gsub("H%x%x", "H")
				C.alf4 = nontra:match("\\4a(&H%x+&)") or sr.color4:match("&H%x%x") .. "&"
			end
			if res.c2 then
				C.col2 = nontra:match("\\2c(&H%x+&)") or sr.color2:gsub("H%x%x", "H")
				C.alf2 = nontra:match("\\2a(&H%x+&)") or sr.color2:match("&H%x%x") .. "&"
			end
		else
			for n = 1, #col do
				local cn = col[n]
				...
				if res.ccol then
					text = addtag3("\\" .. tc .. "c" .. C["col" .. cn], text)
				end
				if res.alfa then
					text = addtag3("\\" .. cn .. "a" .. C["alf" .. cn], text)
				end
			end
		end
		line.text = text
...
function shad3(subs, sel)
	for z = #sel, 1, -1 do
		...
		text = text:gsub("^({[^}]-)\\shad([%d%.]+)", "%1\\xshad%2\\yshad%2"):gsub(STAG, function(tg)
			return duplikill(tg)
		end)
		nontra = text:gsub("\\t%b()", "")
		layer = line.layer
		xshad = tonumber(nontra:match("^{[^}]-\\xshad([%d%.%-]+)")) or 0
		ax = math.abs(xshad)
		yshad = tonumber(nontra:match("^{[^}]-\\yshad([%d%.%-]+)")) or 0
		ay = math.abs(yshad)
		if ax > ay then
			lay = math.floor(ax)
		else
		...
		for l = lay, 1, -1 do
			...
			line2.text = txt:gsub("\\xshad([%d%.%-]+)", function(a)
				xx = tostring(f * a)
				xx = xx:gsub("([%d%-]+%.%d%d)%d+", "%1")
				return "\\xshad" .. xx
			end):gsub("\\yshad([%d%.%-]+)", function(a)
				yy = tostring(f * a)
				yy = yy:gsub("([%d%-]+%.%d%d)%d+", "%1")
				return "\\yshad" .. yy
			end)
...
function space(subs, sel)
	...
	for z = #sel, 1, -1 do
		...
		if not text:match("\\p1") and letrz and #letrz > 1 then
			...
			if fsp then
				sr.spacing = tonumber(fsp)
			end
			fsize = notra:match("^{[^}]-\\fs([%d%.]+)")
			if fsize then
				sr.fontsize = tonumber(fsize)
			end
			...
			if not acalign then
				acalign = text:match("\\an(%d)") or sr.align
			end
			acalign = tostring(acalign)
			if acalign:match("[147]") then
				tags = tags:gsub("\\an%d", ""):gsub("^{", "{\\an" .. acalign + 1):gsub("\\pos%(([%d.-]+)", function(p)
					return "\\pos(" .. round(p + (wtotal / 2) * (scx / 100), 2)
				end)
			end
			...
			for t = 1, #letters do
				...
				if m1 then
					t2 = t2:gsub("\\pos%(([%d.-]+),([%d.-]+)%)%(,[%d.-]+,[%d.-]+", function(a, b)
						return "\\move(" .. a .. "," .. b .. "," .. a + movX .. "," .. b + movY
					end)
				end
				lastxpos = xpos
				l2 = line
...
function splitbreak(subs, sel)
	...
	for z = #sel, 1, -1 do
		...
		-- open split GUI
		if res.splitg then
			lab = "Split by spaces, tags, or any marker."
			if text:match("clip%(m ([^)]+)%)") then
				lab = lab .. "\n(This line will be split by a clip if it has enough points.)"
			end
			if not applytoall then
				P, rez = ADD({
					{ class = "label", label = lab, width = 4 },
			...
			text = text:gsub("\\N", "")
			if P == "Spaces" then
				text = textreplace(text, " ", " \\N")
			end
			if P == "Tags" then
				repeat
					text, r = text:gsub("({\\[^}]*)}( +){(\\[^}]*})", "%2%1%3")
				until r == 0
				text = text:gsub("({\\[^}]*})( +)", "%2%1")
			...
			if P == "Marker" then
				...
				if not mark or mark == "" then
					t_error("No marker set.", 1)
				end
				after = after:gsub(esc(mark), "\\N")
				text = stags .. after
			end
			if rez.remember then
				applytoall = P
			end
			breakit = false
			nmbr = rez.num
		...
		if text:match("\\N") then
			...
			if posY and breakit then
				...
				if align3 > 20 then
					for p = 1, poses do
						table.insert(postab, { posX + (p - 1) * d_hor, posY + (p - 1) * d_ver })
					end
				elseif align3 < 10 then
					...
					seg[1] = seg[1]:gsub("(\\pos%()[%d.-]+,[%d.-]+", "%1" .. P1 .. "," .. P2):gsub(
						"(\\move%()[%d.-]+,[%d.-]+,[%d.-]+,[%d.-]+",
						"%1" .. P1 .. "," .. P2 .. "," .. P1 + movX .. "," .. P2 + movY
					)
				else
					posYm = posY - (poses - 1) * d_ver / 2
					posXm = posX - (poses - 1) * d_hor / 2
					for p = 1, poses do
						table.insert(postab, { posXm + (p - 1) * d_hor, posYm + (p - 1) * d_ver })
					end
					P1, P2 = postab[1][1], postab[1][2]
					seg[1] = seg[1]:gsub("(\\pos%()[%d.-]+,[%d.-]+", "%1" .. P1 .. "," .. P2):gsub(
						"(\\move%()[%d.-]+,[%d.-]+,[%d.-]+,[%d.-]+",
			...
			if not breakit then
				...
				for s = 1, #seg do
					seg[s], sp = seg[s]:gsub(" *$", "")
					ws = aegisub.text_extents(sr, seg[s]:gsub("%b{}", "")) * sfac
					table.insert(wtab, ws)
			...
			line2 = line
			tags = ""
			for it in seg[1]:gmatch(ATAG) do
				tags = tags .. it
			end
			tags = tags:gsub("}{", "")
			count = 0
			...
			for sg = 2, #seg do
				...
				if posY and breakit then
					...
					t2 = t2:gsub("(\\pos%()[%d.-]+,[%d.-]+", "%1" .. P1 .. "," .. P2):gsub(
						"(\\move%()[%d.-]+,[%d.-]+,[%d.-]+,[%d.-]+",
						"%1" .. P1 .. "," .. P2 .. "," .. P1 + movX .. "," .. P2 + movY
					)
				elseif klipos then
				...
				if aftern:gsub("%b{}", "") ~= "" then
					count = count + 1
					t2 = tagmerge(t2):gsub(ATAG, function(tg)
						return duplikill(tg)
					end)
					tags = ""
					for it in t2:gmatch(ATAG) do
						tags = tags .. it
					end
					tags = tags:gsub("}{", "")
					line2.text = t2
				...
				if klipos then
					...
					text = text:gsub("\\move", "\\mpos")
						:gsub("(\\m?pos%()([%d.-]+),([%d.-]+)", function(p, x, y)
							...
							if rot then
								xpos, y = rotxy(rot, xpos, y)
							end
							return p .. round(xpos, 1) .. "," .. y
						end)
						:gsub("\\mpos%(([%d.-]+),([%d.-]+),[%d.-]+,[%d.-]+", function(x, y)
							return "\\move(" .. x .. "," .. y .. "," .. x + movX .. "," .. y + movY
						end)
				end
			end
...
function splitbyclip(klipos, sg, t)
	...
	t = t:gsub("\\pos%([%d.-]+,[%d.-]+", "\\pos(" .. px .. "," .. py):gsub("\\i?clip%b()", ""):gsub(
		"(\\move%()[%d.-]+,[%d.-]+,[%d.-]+,[%d.-]+",
		"%1" .. px .. "," .. py .. "," .. px + movX .. "," .. py + movY
	)
	t = addtag3("\\frz" .. rota, t)
	if valign == 1 then
		ann = "2"
	elseif valign == 3 then
		ann = "8"
	else
		ann = "5"
	end
...
function findpos(text, sg)
	...
	text = text:gsub("\\move", "\\mpos")
		:gsub("(\\m?pos%()([%d.-]+),([%d.-]+)", function(p, x, y)
			...
			local space = ws
			if P == "Tags" and stab[sg - 1] == 1 then
				space = 0
			end
			if align == 1 then
				xpos = round(x + wtab[sg - 1] + space, 2)
			end
			if align == 2 then
				xpos = round(x + wtab[sg - 1] / 2 + wtab[sg] / 2 + space, 2)
			end
			if align == 0 then
				xpos = round(x + wtab[sg] + space, 2)
			end
			xpos2 = xpos
			if rot then
				xpos, y = rotxy(rot, xpos, y)
...
function addtag2(tag, text)
	...
	text = text:gsub("^({\\[^}]-)}", "%1" .. tag .. "}"):gsub(tg .. "[^\\}]+([^}]-)(" .. tg .. "[^\\}]+)", "%2%1")
	return text
end
...
function addtag3(tg, txt)
	...
	elseif no_tf:match("^({[^}]-)" .. tgt .. val) then
		txt = txt:gsub("^({[^}]-)" .. tgt .. val .. "[^\\}]*", "%1" .. tg)
...
function stylechk(subs, sn)
	...
	if sr == nil then
		t_error("Style '" .. sn .. "' doesn't exist.", 1)
	end
	return sr
end
...
function retextmod(orig, text)
	...
	if v1 ~= v2 then
		logg("Something went wrong with the text...")
		logg(v1)
		logg(v2)
	end
	return t2
...
function textmod(orig, text)
	...
	if not ltrmatches then
		logg("text: " .. text .. "\nvisible: " .. vis)
		logg(
			"If you're seeing this, something really weird is happening with the re module.\nTry this again or rescan Autoload."
		)
	end
...
function duplikill(tagz)
	...
	tagz = tagz:gsub("\\t%b()", function(t)
		return t:gsub("\\", "|")
	end)
	for i = 1, #tags1 do
		tag = tags1[i]
		repeat
			tagz, c = tagz:gsub("|" .. tag .. "[%d%.%-]+([^}]-)(\\" .. tag .. "[%d%.%-]+)", "%1%2")
		until c == 0
		repeat
			tagz, c = tagz:gsub("\\" .. tag .. "[%d%.%-]+([^}]-)(\\" .. tag .. "[%d%.%-]+)", "%2%1")
		until c == 0
	end
	tagz = tagz:gsub("\\1c&", "\\c&")
	for i = 1, #tags2 do
		tag = tags2[i]
		repeat
			tagz, c = tagz:gsub("|" .. tag .. "&H%x+&([^}]-)(\\" .. tag .. "&H%x+&)", "%1%2")
		until c == 0
		repeat
			tagz, c = tagz:gsub("\\" .. tag .. "&H%x+&([^}]-)(\\" .. tag .. "&H%x+&)", "%2%1")
		until c == 0
	end
	repeat
		tagz, c = tagz:gsub("\\fn[^\\}]+([^}]-)(\\fn[^\\}]+)", "%2%1")
	until c == 0
	repeat
		tagz, c = tagz:gsub("(\\[ibusq])%d(.-)(%1%d)", "%2%3")
	until c == 0
	repeat
		tagz, c = tagz:gsub("(\\an)%d(.-)(%1%d)", "%3%2")
	until c == 0
	tagz = tagz:gsub("(|i?clip%(%A-%))(.-)(\\i?clip%(%A-%))", "%2%3")
		:gsub("(\\i?clip%b())(.-)(\\i?clip%b())", function(a, b, c)
			if a:match("m") and c:match("m") or not a:match("m") and not c:match("m") then
				return b .. c
			else
				return a .. b .. c
			end
...
function extrakill(text, o)
	...
	for i = 1, #tags3 do
		...
		if o == 2 then
			repeat
				text, c = text:gsub("(\\" .. tag .. "[^\\}]+)([^}]-)(\\" .. tag .. "[^\\}]+)", "%3%2")
			until c == 0
		else
			repeat
				text, c = text:gsub("(\\" .. tag .. "[^\\}]+)([^}]-)(\\" .. tag .. "[^\\}]+)", "%1%2")
			until c == 0
		end
	end
	repeat
		text, c = text:gsub("(\\pos[^\\}]+)([^}]-)(\\move[^\\}]+)", "%1%2")
	until c == 0
	repeat
		text, c = text:gsub("(\\move[^\\}]+)([^}]-)(\\pos[^\\}]+)", "%1%2")
	until c == 0
	return text
end
...
function necrohell()
	...
	repeat
		if Pr == "clip2fax/frz" then
			nekrohelp = faxhelp
		end
		if Pr == "necroscopy" then
			nekrohelp = necrohelp
		end
		if Pr == "copy tags/text" then
			nekrohelp = tagsthelp
		end
		if Pr == "copy colours" then
			nekrohelp = callhelp
		end
		if Pr == "3D shadow" then
			nekrohelp = shadehelp
		end
		if Pr == "split letters" then
			nekrohelp = splathelp
		end
		if Pr == "split by \\N" then
			nekrohelp = breakhelp
		end
		Pr = aegisub.dialog.display({
			{ width = 32, height = 10, class = "textbox", value = nekrohelp },
			{ x = 36, height = 10, class = "label", label = "NecrosCopy\nversion " .. script_version },
		}, {
			"clip2fax/frz",
			"necroscopy",
			"copy tags/text",
...
function necroscopy(subs, sel)
	...
	GUI = {
		...
		},
		{
			...
			label = "autogradient",
			value = res.grad,
			hint = "automatically gradient \\fax",
		},
		-- copy
		{
			x = 2,
			...
			label = "bottom up",
			value = res.bot,
			hint = "copy from the bottom up [all copy functions]",
		},
		{
			x = 3,
			y = 0,
		...
		},
		{
			...
			class = "checkbox",
			name = "vis",
			label = "only visible text",
			value = res.vis,
			hint = "no inline tags/comments [copy text]",
		},
		-- colours
	...
	if P == "split letters" then
		sel = space(subs, sel)
	end
	if P == "split by \\N" then
		sel = splitbreak(subs, sel)
	end
	if P == "split GUI" then
		res.splitg = true
		sel = splitbreak(subs, sel)
