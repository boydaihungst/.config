...
.br
--report-per-command' -d 'Report the following statistics for each command after the benchmark finishes…'
...
complete -c pgbench -l failures-detailed -d 'Report failures in per-transaction and aggregation logs, as well as in the ma…'
complete -c pgbench -l log-prefix -d 'Set the filename prefix for the log files created by --log'
complete -c pgbench -l max-tries -d 'Enable retries for transactions with serialization/deadlock errors and set th…'
...
complete -c pgbench -l verbose-errors -d 'Print messages about all errors and failures (errors without retrying) includ…'
...
complete -c pgbench -l report-per-command
