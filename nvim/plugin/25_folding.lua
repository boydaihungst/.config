...
M.config = {
  ...
  enabled = true,
  methods = { 'lsp', 'treesitter', 'indent', 'minigitlog' },
}
...
local fold_methods = {
  minigitlog = function(lnum, bufnr)
    if bufnr and vim.api.nvim_buf_is_valid(bufnr) and (vim.bo[bufnr].filetype == 'git' or vim.bo[bufnr].filetype == 'diff') then
      return MiniGit.diff_foldexpr(lnum)
    end
  end,
  ...
  treesitter = function(lnum, bufnr)
    ...
    if treesitter.has_parser(bufnr, 'folds') and treesitter.is_enabled(bufnr) then return vim.treesitter.foldexpr(lnum) end
  end,
  indent = function(lnum, bufnr)
    if not lnum then lnum = vim.v.lnum end
    if not bufnr then bufnr = vim.api.nvim_get_current_buf() end
    return vim.api.nvim_buf_get_lines(bufnr, lnum - 1, lnum, false)[1]:match '^%s*$' and '=' or math.floor(vim.fn.indent(lnum) / vim.bo[bufnr].shiftwidth)
  end,
}
...
function M.is_enabled(bufnr)
  local enabled = M.config and M.config.enabled
...
function M.foldexpr(lnum)
  ...
  if M.is_enabled(bufnr) then
    for _, method in ipairs(M.config and M.config.methods or {}) do
...
function M.info(bufnr)
  ...
  local methods = M.config and M.config.methods or {}
...
function M.setup()
  vim.api.nvim_create_user_command('FoldInfo', function() M.info() end, { desc = 'Display folding information' })
  ...
  vim.api.nvim_create_autocmd('LspDetach', {
    ...
    callback = function(args)
      ...
      for _, client in pairs(vim.lsp.get_clients { bufnr = args.buf }) do
        if client.id ~= args.data.client_id and client:supports_method('textDocument/foldingRange', args.buf) then return end
      end
